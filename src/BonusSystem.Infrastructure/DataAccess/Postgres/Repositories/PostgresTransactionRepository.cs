...
public class PostgresTransactionRepository : ITransactionRepository
{
    ...
    private readonly ILogger<PostgresTransactionRepository> _logger;

    public PostgresTransactionRepository(BonusSystemDbContext dbContext, ILogger<PostgresTransactionRepository> logger)
    {
    ...
    public async Task<IEnumerable<TransactionDto>> GetAllAsync()
    {
        try
        {
            var entities = await _dbContext.Transactions.AsNoTracking().ToListAsync();
    ...
    public async Task<TransactionDto?> GetByIdAsync(Guid id)
    {
        try
        {
            var entity = await _dbContext.Transactions.AsNoTracking()
    ...
    public async Task<bool> UpdateAsync(TransactionDto dto)
    {
        try
        {
            var entity = await _dbContext.Transactions.FindAsync(dto.Id);
            if (entity == null)
            {
                return false;
            }
    ...
    public async Task<IEnumerable<TransactionDto>> GetTransactionsByUserIdAsync(Guid userId)
    {
        try
        {
            var entities = await _dbContext.Transactions.AsNoTracking()
    ...
    public async Task<decimal> GetTotalBonusCirculationAsync()
    {
        try
        {
            ...
            var totalEarned = await _dbContext.Transactions.AsNoTracking()
                .Where(t => t.Type == TransactionType.Earn && t.Status == TransactionStatus.Completed)
                .SumAsync(t => t.Amount);
            
            // Sum of all Spend transactions
            var totalSpent = await _dbContext.Transactions.AsNoTracking()
    ...
    public async Task<int> GetTotalTransactionsCountAsync()
    {
        try
        {
            return await _dbContext.Transactions.AsNoTracking().CountAsync();
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<TransactionDto>> GetActiveTransactionsForCompanyAsync(Guid companyId)
    {
        try
        {
            // Get all completed transactions for this company
            var entities = await _dbContext.Transactions.AsNoTracking()
    ...
    public async Task ProcessExpirationAsync(DateTime expirationDate)
    {
        ...
        try
        {
            ...
            foreach (var user in usersWithBonus)
            {
                ...
                var expirationTransaction = new TransactionEntity
                {
                    Id = Guid.NewGuid(),
                    UserId = user.Id,
                ...
                await _dbContext.Transactions.AddAsync(expirationTransaction);
